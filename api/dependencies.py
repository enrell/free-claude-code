...
from providers.opencode_go import OPENCODE_GO_BASE_URL, OpenCodeGoProvider
...
def _create_provider_for_type(provider_type: str, settings: Settings) -> BaseProvider:
    ...
    if provider_type == "opencode_go":
        if not settings.opencode_go_api_key or not settings.opencode_go_api_key.strip():
            raise AuthenticationError(
                "OPENCODE_GO_API_KEY is not set. Add it to your .env file. "
                "Get a key at https://opencode.ai/zen/go"
            )
        config = ProviderConfig(
    ...
    logger.error(
        "Unknown provider_type: '{}'. Supported: 'nvidia_nim', 'open_router', 'lmstudio', 'llamacpp', 'opencode_go'",
        provider_type,
    )
    raise ValueError(
